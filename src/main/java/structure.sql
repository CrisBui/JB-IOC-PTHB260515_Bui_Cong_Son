...
CREATE TABLE borrow_cards(
                             ...
                             return_dateline TIMESTAMP NOT NULL,
...
BEGIN
    RETURN QUERY
        SELECT card_id, book_title, borrower_name, borrow_date, return_dateline, quantity, status
        FROM borrow_cards;
...
AS $$
BEGIN
    INSERT INTO borrow_cards(book_title, borrower_name, borrow_date, return_dateline, quantity, status)
...
CREATE OR REPLACE FUNCTION func_get_borrow_cards_by_borrower_name_v1(p_borrower_name VARCHAR)
    RETURNS TABLE(
                     card_id INT,
                     book_title VARCHAR,
                     borrower_name VARCHAR,
...
BEGIN
    RETURN QUERY
        SELECT bc.card_id, bc.book_title, bc.borrower_name, bc.borrow_date, bc.return_dateline, bc.quantity, bc.status
        FROM borrow_cards bc
        WHERE bc.borrower_name = p_borrower_name;
end;
$$;
--  2.3.1 func phuc vu tim kiem gan dung
...
BEGIN
    ...
    SET borrower_name = p_borrower_name,
        ...
        return_dateline = p_return_deadline,
        quantity  = p_quantity,
        status = p_status
